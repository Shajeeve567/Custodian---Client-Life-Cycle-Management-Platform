...
public class NextActionServiceTests
{
    ...
    private static NextActionService CreateService(
        ...
        new(
            ...
            conditionService,
            documentClient,
            new SlaCalculator(new StallDetectionService(Microsoft.Extensions.Options.Options.Create(
                new Custodian.Workflow.Configuration.SlaOptions { DefaultOverdueHours = 72 }))),
            gateEvaluator,
            stallService ?? NotStalled(),
            timeProvider ?? new FakeTimeProvider(Now),
            NullLogger<NextActionService>.Instance);

    private static IStallService NotStalled()
    {
        var stall = new Mock<IStallService>();
        stall.Setup(s => s.GetStallStatusAsync(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
        return stall.Object;
    }
    ...
    public async Task InjectedTimeProvider_DrivesOverduePromotion()
    {
        ...
        await db.ClientActions.AddAsync(new ClientAction
        {
            ...
            ActivatedAt = Now.UtcDateTime.AddDays(-1)
        });
        await db.SaveChangesAsync();
